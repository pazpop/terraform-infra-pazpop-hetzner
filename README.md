...
- `Content-Security-Policy` sur le middleware `secure-headers` (`docker/traefik/dynamic/middlewares.yml`), appliquée à tous les jeux/portail routés par Traefik. `'unsafe-eval'` requis par le lecteur de musique d'arcadepipe (`lib/libopenmpt.worklet.js` : libopenmpt compilé en **WebAssembly** par Emscripten, binaire embarqué dans le fichier — pas de `.wasm` séparé ; `'wasm-unsafe-eval'` serait une alternative plus étroite, non testée) ; `'unsafe-inline'` sur `style-src` requis pour le `<style>` inline généré par le portail ; `googletagmanager.com`/`google-analytics.com` sur `script-src`/`connect-src` requis pour Google Analytics (arcadepipe) — le tag est injecté dynamiquement avec un `src` externe (jamais de script inline, donc pas besoin de `'unsafe-inline'` sur `script-src`), et **seulement après consentement** du visiteur (bandeau côté jeu, `js/consent.js` → `js/analytics.js`). Testé en réel (navigateur vierge, site en direct, avant et après consentement, envois simulés) le 2026-09-18 : zéro violation. Si `www.google-analytics.com` est bloqué chez le visiteur (bloqueur de pub), gtag bascule sur `www.google.com/g/collect`, non autorisé par cette CSP : erreurs console sans conséquence pour le jeu.
- Rate-limiting Traefik (middleware `rate-limit`, `docker/traefik/dynamic/middlewares.yml`) : 20 req/s par IP source (burst 40) — fail2ban ne protège que SSH, rien côté 80/443 sans ce middleware. Appliqué aux routeurs qui exposent une vraie API (écritures en base, `arcadepipe-api`) et au portail. **Pas** sur `arcadepipe-web` (fichiers statiques) : ArcadePipe est du JS vanilla sans bundler, une quarantaine de fichiers JS chargés séparément par page — un simple second onglet ou une IP partagée dépassait le burst sans rien d'anormal, provoquant des 429 sur les fichiers musique (silence audio permanent côté jeu, voir GAMEPLAY.md du repo arcadepipe). Limiter par IP un fichier statique protège contre un risque quasi nul au prix de casser un usage légitime courant.
